...
import PartnerInsert from "./PartnersInsert"
import PartnerDelete from "./PartnersDelete"
import PartnerEdit from "./PartnersEdit"
...
const PartnersHome = () => {
  const columns = [
    ...
    },
    {
        ...
        header: "封面图片",
        cell: ({row}) => (
            <Image src={row.getValue("logo_url")} alt='' width={250} height={250}/>
        )
    },
    {
        accessorKey: "ctime",
    ...
    },
    {
      ...
      cell: ({ row }) => {
        ...
        return <PartnerEdit info={data.find(i => i.id === id)} semaphore={setSemaphore}/>
      },
    },
    {
      accessorKey: "delete",
      ...
      cell: ({ row }) => {
        ...
        return <PartnerDelete info={row} semaphore={setSemaphore}/>
      },
    },
]
  ...
  React.useEffect(() => {
    const fetchCases = async () => {
        const res = await instance.get('http://http://120.76.205.116/:9000/partners/get_all_for_admin')
        if (res.data.code === 2) {
  ...
  return (
    <div className="w-full flex flex-col py-3 px-8">
      <h2 className='text-4xl'>合作伙伴管理</h2>
      <div className="flex items-center py-4">
        <Input
          placeholder="搜索合作伙伴信息..."
      ...
      <div>
      <PartnerInsert title='新增案例' semaphore={setSemaphore} className='w-1/2'/>
      </div>
